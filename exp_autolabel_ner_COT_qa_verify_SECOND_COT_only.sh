#!/bin/bash
#SBATCH --job-name=sbs		# job name
#SBATCH --mem 30G		# memory size
#SBATCH -n 4 			# number of cpu
#SBATCH -N 1 			# number of node
##SBATCH --gres=gpu:1		# number of gpu
##SBATCH -C H100|A100|A100-80G|V100			# name of gpu that you want to use
##SBATCH -x gpu-4-01,gpu-4-04,gpu-4-11,gpu-4-13	# name of node that you don't want to request for
#SBATCH -p short		# maximum running time, long (7 days) or short (1 day)
##SBATCH -d afterany:375534
##SBATCH -o H-output     	# STDOUT
##SBATCH -e H-error      	# STDERR
##SBATCH --mail-type=BEGIN,END	# notifications for job done & fail
##SBATCH --mail-user=dzhang5@wpi.edu 

# for entity_type in "Food" "Location" "Symptom" "Keyword"
# # for entity_type in "Food"
# do
size=${1:-1.0}
agg_method=${2:-"expert"}
model_name=${3:-"google/flan-t5-small"}
few_shot_selection=${4:-"semantic_similarity"}
temperature=${5:-0.1}
entity_type=${6:-"Food"}
verify=${7:-"True"}
cot_col=${8:-"one_step"}
label_version=${9:-'v1'}
task_version=${10:-'v1'}
verify_few_shot_selection=${11:-"label_diversity_similarity"}
tweet_few_shot_selection=${12:-"label_diversity_similarity"}
second_verify_few_shot_selection=${13:-"label_diversity_similarity"}
few_shot_path='/scratch/dzhang5/LLM/TWEET-FID/'${size}'.'${agg_method}'.devfortrain.short.csv'
data_path='/scratch/dzhang5/LLM/TWEET-FID/expert.traintestfortest.csv'
output_dir='/scratch/dzhang5/LLM/TWEET-FID/traintestfortest-results-autolabel-ner-qa-'${agg_method}
tweet_output_dir='/scratch/dzhang5/LLM/TWEET-FID/traintestfortest-results-autolabel-'${agg_method}
text_column="context"
label_column=${entity_type}_answer
example_selection_label_column=has_${entity_type}
if [[ "$cot_col" == "one_step" ]]; then
explanation_column=${entity_type}_explanation
elif [[ "$cot_col" == "two_step" ]]; then
explanation_column=Two_step_${entity_type}_explanation
else
explanation_column=word_explanation
fi
label_symbol="^^^^"
token_path="/home/dzhang5/.cache/huggingface/token"
few_shot_num=8

use_current_explanation=False
use_ground_explanation=False

cache=False
console_output=False

if [[ "$few_shot_num" == 8 ]]; then
output_dir=${output_dir}/${temperature}/${size}/lv_${label_version}_tv_${task_version}
tweet_output_dir=${tweet_output_dir}/${temperature}/${size}/lv_${label_version}_tv_${task_version}
directory=NER_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}
output_file=NER_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/out_AutoLabel_NER_COT_QA_second_verification_COT_${cot_col}_${trans_model_name}_${few_shot_selection}_${entity_type}_${data_name}_${size}_${agg_method}.ipynb
else
output_dir=${output_dir}/${temperature}/${size}/lv_${label_version}_tv_${task_version}/few_${few_shot_num}
tweet_output_dir=${tweet_output_dir}/${temperature}/${size}/lv_${label_version}_tv_${task_version}/few_${few_shot_num}
directory=NER_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/few_${few_shot_num}
output_file=NER_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/few_${few_shot_num}/out_AutoLabel_NER_COT_QA_second_verification_COT_${cot_col}_${trans_model_name}_${few_shot_selection}_${entity_type}_${data_name}_${size}_${agg_method}.ipynb
fi

trans_model_name=$(echo "$model_name" | sed 's/\//_/g')
data_name=$(basename "$data_path")


# Check if the directory exists
if [ ! -d "$directory" ]; then
    # Create the directory
    mkdir -p "$directory"
fi

papermill \
AutoLabel_NER_COT_QA_second_verification_COT.ipynb \
${output_file} \
-k py3.9-visa \
-p few_shot_path ${few_shot_path} \
-p data_path ${data_path} \
-p output_dir ${output_dir} \
-p tweet_output_dir ${tweet_output_dir} \
-p model_name ${model_name} \
-p label_column ${label_column} \
-p text_column ${text_column} \
-p few_shot_num ${few_shot_num} \
-p example_selection_label_column ${example_selection_label_column} \
-p explanation_column ${explanation_column} \
-p label_symbol ${label_symbol} \
-p few_shot_selection ${few_shot_selection} \
-p verify_few_shot_selection ${verify_few_shot_selection} \
-p tweet_few_shot_selection ${tweet_few_shot_selection} \
-p second_verify_few_shot_selection ${second_verify_few_shot_selection} \
-p use_current_explanation ${use_current_explanation} \
-p use_ground_explanation ${use_ground_explanation} \
-p token_path ${token_path} \
-p cache ${cache} \
-p console_output ${console_output} \
-p temperature ${temperature} \
-p verify ${verify} \
-p label_version ${label_version} \
-p task_version ${task_version}

# done