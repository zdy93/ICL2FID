...
few_shot_path='/scratch/dzhang5/LLM/TWEET-FID/'${size}'.'${agg_method}'.devfortrain.csv'
...
if [[ "$verify" == "True" ]]; then
data_path='/scratch/dzhang5/LLM/TWEET-FID/LONG_traintestfortest-results-autolabel-ner-qa-'${agg_method}'/'${temperature}'/'${size}'/lv_'${label_version}'_tv_'${task_version}'/'${last_model_name}'_'${check_mode}'_'${word_few_shot_selection}'_COT_'${word_explanation_column}'_cur_'${word_use_current_explanation}'_ground_'${word_use_ground_explanation}'_aggregated_final_COT_'${v_tail}'expert.traintestfortest.csv'
else
...
output_dir='/scratch/dzhang5/LLM/TWEET-FID/LONG_traintestfortest-results-autolabel-'${agg_method}
...
output_file=TC_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/out_AutoLabel_COT_COT_COT_check_${trans_model_name}_${check_mode}_${few_shot_selection:0:3}_${word_few_shot_selection:0:3}_${verify_few_shot_selection:0:3}_${data_name}_current_${use_current_explanation}_ground_${use_ground_explanation}.ipynb
