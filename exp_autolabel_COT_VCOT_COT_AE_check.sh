...
verify=${6:-"True"}
check_mode=${7:-"strict"}
word_explanation_column="entity_explanation"
label_version=${9:-'v1'}
task_version=${10:-'v1'}
...
data_path_par='/scratch/dzhang5/LLM/TWEET-FID/traintestfortest-results-autolabel-ner-ae-'${agg_method}'/'${temperature}'/'${size}'/lv_'${label_version}'_tv_'${task_version}'/few_'${few_shot_num}
...
output_file=TC_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/few_${few_shot_num}/out_AutoLabel_COT_COT_COT_check_${trans_model_name}_${check_mode}_${few_shot_selection:0:3}_${word_few_shot_selection:0:3}_${verify_few_shot_selection:0:3}_${data_name}_current_${use_current_explanation}_ground_${use_ground_explanation}.ipynb



if [[ "$verify" == "True" ]]; then
data_path=${data_path_par}'/'${last_model_name}'_'${check_mode}'_'${word_few_shot_selection}'_COT_AE_'${word_explanation_column}'_cur_'${word_use_current_explanation}'_ground_'${word_use_ground_explanation}'_aggregated_final_COT_'${v_tail}'expert.traintestfortest.csv'
else
data_path=${data_path_par}'/'${last_model_name}'_'${check_mode}'_'${word_few_shot_selection}'_COT_AE_'${word_explanation_column}'_cur_'${word_use_current_explanation}'_ground_'${word_use_ground_explanation}'_aggregated_expert.traintestfortest.csv'
fi
