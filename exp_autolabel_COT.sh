...
if [[ "$verify" == "True" ]]; then
data_path='/scratch/dzhang5/LLM/TWEET-FID/traintestfortest-results-autolabel-ner-qa-'${agg_method}'/'${temperature}'/'${size}'/'${last_model_name}'_'${check_mode}'_'${few_shot_selection}'_aggregated_final_expert.traintestfortest.csv'
else
data_path='/scratch/dzhang5/LLM/TWEET-FID/traintestfortest-results-autolabel-ner-qa-'${agg_method}'/'${temperature}'/'${size}'/'${last_model_name}'_'${check_mode}'_'${few_shot_selection}'_aggregated_expert.traintestfortest.csv'
fi
...
output_file=TC_output/${agg_method}/${size}/lv_${label_version}_tv_${task_version}/out_AutoLabel_COT_${trans_model_name}_${check_mode}_${few_shot_selection}_${data_name}_current_${use_current_explanation}_ground_${use_ground_explanation}.ipynb
